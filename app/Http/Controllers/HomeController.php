...
class HomeController extends Controller{
    ...
    public function index(Request $request){
        ...
        $entradasHoje = Historico::where('tipo', 'entrada')
            ->whereDate('created_at', now())
            ->sum('quantidade');
        ...
        for ($i = 4; $i >= 0; $i--) {
            ...
            $graficoEntradas[] = Historico::whereDate('created_at', $data)
                ->where('tipo', 'entrada')
                ->sum('quantidade');

            $graficoSaidas[] = Historico::whereDate('created_at', $data)
                ->where('tipo', 'saida')
                ->sum('quantidade');
        }

        // RANKING
        $ranking = Historico::select(
                'acessorio_id',
