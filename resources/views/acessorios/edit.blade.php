            ...
            <form action="{{ route('acessorios.update', $acessorio->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="quantidade" value="{{ old('quantidade', $acessorio->quantidade) }}"
                            class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Cor</label>
                        <input type="text" name="cor" value="{{ old('cor', $acessorio->cor) }}"
                            class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Preço</label>
                        <input type="number" step="0.01" name="preco" value="{{ old('preco', $acessorio->preco) }}"
                            class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                </div>
